  ...
  const payload = {
    "requestBody": {
      "dataSet": {
        ...
        "entity": {
          "fieldset": {
            "list": "NUNOTA, CODTIPOPER, CODTIPVENDA, CODPARC, CODVEND, VLRNOTA, DTNEG, VOLUME, TIPMOV"
          }
        }
      }
    }
...
async function upsertCabecalhoNota(
  ...
  for (const cabecalho of cabecalhos) {
    try {
      ...
      if (exists) {
        // Converter VOLUME para número no UPDATE também
        let volumeNumerico = null;
        if (cabecalho.VOLUME) {
          const volumeStr = String(cabecalho.VOLUME).trim();
          const volumeParsed = parseFloat(volumeStr);
          if (!isNaN(volumeParsed)) {
            volumeNumerico = volumeParsed;
          }
        }
        ...
        // Converter VOLUME para número, ignorando valores não numéricos
        let volumeNumerico = null;
        if (cabecalho.VOLUME) {
          const volumeStr = String(cabecalho.VOLUME).trim();
          const volumeParsed = parseFloat(volumeStr);
          if (!isNaN(volumeParsed)) {
            ...
            console.log(`⚠️ [Sync] Valor não numérico em VOLUME para NUNOTA ${cabecalho.NUNOTA}: '${volumeStr}' - será ignorado`);
          }
        }
        ...
        await connection.execute(
          `INSERT INTO AS_CABECALHO_NOTA (
            ...
            CODVEND, VLRNOTA, DTNEG, VOLUME, TIPMOV,
            SANKHYA_ATUAL, DT_ULT_CARGA, DT_CRIACAO
          ) VALUES (
            :idSistema, :nunota, :codtipoper, :codtipvenda, :codparc,
            :codvend, :vlrnota, ${dtnegFormatted ? "TO_DATE(:dtneg, 'DD/MM/YYYY')" : 'NULL'}, :volume, :tipmov,
